...
instead of a single monolithic transform.
"""
...
def build_hierarchy() -> Callable[..., dict[str, Any]]:
    ...
    def _worker(atom_arrays: dict[str, np.ndarray]) -> dict[str, Any]:
...
def derive_atom_features() -> Callable[..., dict[str, np.ndarray]]:
...
def derive_bond_edges() -> Callable[..., dict[str, np.ndarray]]:
    """Build atom-level bond edges from the structure's connectivity + CCD chemistry."""
...
def derive_residue_features() -> Callable[..., dict[str, np.ndarray]]:
...
def derive_chain_features() -> Callable[..., dict[str, np.ndarray]]:
    """Derive chain-level features (``entity_type`` from ``molecule_type_id``)."""

    def _worker(hierarchy: dict[str, Any]) -> dict[str, np.ndarray]:
...
def assemble_cifmol() -> Callable[..., "BioMolDict"]:
    ...
    def _worker(  # noqa: PLR0913
        ...
        return CIFMol(
            ...
            residue_container=residues,
            chain_container=chains,
